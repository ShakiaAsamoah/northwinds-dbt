with hubspot_companies as (
    ...
    select business_id as hubspot_company_id, null as rds_company_id, name from hubspot_companies union all
    select null as hubspot_company_id, company_id as rds_company_id, companyname from rds_companies
), deduped as (
    select 
 ...
 select {{ dbt_utils.surrogate_key(['deduped.name']) }} as company_pk, hubspot_company_id, rds_company_id, deduped.name, address,
   city, postal_code, country from deduped
